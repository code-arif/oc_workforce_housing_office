...
use App\Mail\Tenant\AdminSignedMail;
...
class LeaseDocumentController extends Controller
{
    ...
    /**
     * Handle admin signature submission for a lease document
     */
    public function signAdmin(Request $request, $id)
    {
        $request->validate([
        ...
        try {
            ...
            $document->update([
                'admin_signature' => $request->signature,
                'admin_signed_at' => now(),
                'status'          => $document->tenant_signature
                    ? 'signed'
                    : 'pending_signatures',
            ]);
            ...
            if ($document->tenant_signature) {
                ...
                if ($lease->tenant->status !== 'approved') {
                    $lease->tenant->update([
                        'status' => 'approved',
                    ]);
                }

                $mailable = new LeaseFullySignedMail($lease);
                ...
                $mailable = new AdminSignedMail($lease);
            }

            Mail::to($lease->tenant->email)->queue($mailable);
            ...
            Log::error('Error saving admin signature', [
                'lease_document_id' => $id,
                'message'           => $e->getMessage(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to save admin signature',
            ], 500);
        }
    }
