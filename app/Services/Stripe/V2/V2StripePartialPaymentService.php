...
class V2StripePartialPaymentService
{
    ...
    public function verifyPayment($sessionId): array
    {
        try {
            ...
            Log::error('[StripePartialPayment] Payment verification failed: ' . $e->getMessage());
    ...
    private function processPayment($metadata, $sessionOrIntent): array
    {
        ...
        try {
            ...
            if (!$invoiceId) {
                DB::rollBack();
                Log::warning('[StripePartialPayment] Payment processing skipped: Missing invoice_id in metadata (Likely a Stripe test webhook)', [
            ...
            if ($existingPayment) {
                DB::rollBack();
                Log::info('[StripePartialPayment] Payment already processed, skipping', [
            ...
            $payment = Payment::create([
                ...
                'note' => sprintf(
                    '%s partial payment via Stripe - Invoice %s',
            ...
            Log::info('[StripePartialPayment] Payment record created in database', [
            ...
            Transaction::create([
                ...
                'description' => sprintf(
                    '%s partial payment for Invoice %s - %s (via Stripe Connect → %s)',
            ...
            Log::info('[StripePartialPayment] Payment processed successfully via Stripe Connect', [
                'payment_id' => $payment->id,
                'invoice_id' => $invoice->id,
                'connected_account' => $connectedAccountId,
                'amount' => $baseAmount,
            ...
            DB::rollBack();
            Log::error('[StripePartialPayment] Payment processing failed: ' . $e->getMessage(), [
    ...
    private function sendPaymentEmails($payment, $invoice, $tenant, $metadata): void
    {
        try {
            ...
            Log::error('[StripePartialPayment] Failed to send payment emails: ' . $e->getMessage(), [
    ...
    {
        $endpoint_secret = config('services.stripe.partial_webhook_secret') ?? config('services.stripe.webhook_secret');
        ...
        try {
            ...
            Log::error('[StripePartialPayment] Webhook signature verification failed: ' . $e->getMessage());
        ...
        switch ($event->type) {
            case 'payment_intent.succeeded':
                ...
                Log::info('[StripePartialPayment] PaymentIntent succeeded via webhook', [
                    'intent_id' => $intent->id,
                ]);

                $result = $this->processPayment($intent->metadata, $intent);
                if (!$result['success']) {
                    Log::error('[StripePartialPayment] Webhook payment processing failed for intent', [
                        'intent_id' => $intent->id,
            ...
            case 'payment_intent.payment_failed':
                $intent = $event->data->object;
                Log::warning('[StripePartialPayment] PaymentIntent failed via webhook', [
                    'intent_id' => $intent->id,
            ...
            case 'payment_intent.requires_action':
                $intent = $event->data->object;
                Log::info("[StripePartialPayment] PaymentIntent lifecycle event: {$event->type}", [
                    'intent_id' => $intent->id,
            ...
            case 'setup_intent.succeeded':
                $intent = $event->data->object;
                Log::info("[StripePartialPayment] SetupIntent lifecycle event: {$event->type}", [
            ...
            case 'checkout.session.completed':
                ...
                Log::info('[StripePartialPayment] Checkout session completed via webhook', [
                    'session_id' => $session->id,
                ...
                if ($session->payment_status === 'paid') {
                    ...
                    if (!$result['success']) {
                        Log::error('[StripePartialPayment] Webhook payment processing failed', [
                            'session_id' => $session->id,
                            'error' => $result['message']
                        ]);
                    }
            ...
            case 'checkout.session.async_payment_succeeded':
                $session = $event->data->object;
                Log::info('[StripePartialPayment] Checkout session async payment succeeded via webhook', [
                    'session_id' => $session->id,
                    'payment_status' => $session->payment_status,
                ]);
                ...
                $result = $this->processPayment($session->metadata, $session);
                if (!$result['success']) {
                    Log::error('[StripePartialPayment] Webhook async payment processing failed', [
                        'session_id' => $session->id,
                        'error' => $result['message']
                    ]);
                }
            ...
            case 'checkout.session.async_payment_failed':
                $session = $event->data->object;
                Log::warning('[StripePartialPayment] Checkout session async payment failed via webhook', [
                    'session_id' => $session->id,
                    'payment_status' => $session->payment_status,
            ...
            case 'transfer.created':
                ...
                Log::info('[StripePartialPayment] Transfer created to connected account', [
            ...
            default:
                Log::info('[StripePartialPayment] Unhandled webhook event type: ' . $event->type);
        }
