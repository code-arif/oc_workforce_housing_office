            ...
            <div class="main-container container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-4 col-lg-5">
                        ...
                        <div class="card tenant-profile-card">
                            <div class="card-body text-center pt-4 pb-4">
                                ...
                                <p class="text-muted mb-3">
                                    <i class="fe fe-calendar me-1"></i> Tenant since
                                    {{ $tenant->created_at->format('M d, Y') }}
                                </p>
                                {{-- <div class="d-flex justify-content-center gap-2 mb-3">
                                    <span
                                        class="badge bg-{{ $tenant->status === 'approved' || $tenant->status === 'active' ? 'success' : ($tenant->status === 'pending' ? 'warning' : 'secondary') }} px-3 py-2">
                                ...
                                <div class="d-flex justify-content-center gap-2 mb-3">
                                    <span
                                        class="badge bg-{{ $tenant->status === 'approved' || $tenant->status === 'active' ? 'success' : ($tenant->status === 'pending' ? 'warning' : 'secondary') }} p-3 rounded-pill">
                                        {{ ucfirst($tenant->status) }}
                                    </span>
                                    @if($activeLease)
                                ...
                                <div class="d-flex justify-content-center gap-2 mb-3 flex-wrap">
                                    <button type="button" class="btn btn-secondary d-inline-flex align-items-center"
                                        title="View application details" id="viewApplicationBtn"
                                        data-tenant-id="{{ $tenant->id }}"
                                        onclick="viewApplicationDetails({{$tenant->application_id}})">
                                    ...
                                    <button type="button" class="btn btn-outline-primary d-inline-flex align-items-center"
                                        title="Resend Password Setup Email" onclick="resendSetupEmail({{ $tenant->id }})">
                                ...
                                <div class="row text-center">
                                    ...
                                    <div class="col-4 border-start border-end">
                                        <div class="fw-bold text-success">
                                            ${{ number_format($invoiceStats['paid_amount'] ?? 0, 2) }}</div>
                                        <small class="text-muted">Paid</small>
                                    </div>
                                    <div class="col-4">
                                        <div
                                            class="fw-bold text-{{ ($invoiceStats['balance_due'] ?? 0) > 0 ? 'danger' : 'muted' }}">
                                            ${{ number_format($invoiceStats['balance_due'] ?? 0, 2) }}</div>
                        ...
                        @if($tenant->leases && $tenant->leases->count() > 0)
                                    ...
                                    <div class="bed-assignments-list">
                                        @foreach ($tenant->leases as $lease)
                                            @forelse ($lease->assignments as $assigned)
                                                <div
                                                    class="bed-assignment-item {{ $assigned->is_current ? 'current-bed' : 'past-bed' }}">
                                                    <div class="bed-icon">
                                                        @if($assigned->bed_id)
                                                        ...
                                                        @if($assigned->bed_id)
                                                            <div class="bed-label">
                                                                ...
                                                                @if($assigned->is_current && $assigned->actual_move_in)
                                                                    <br> <small class="bed-subtext">Move In:
                                                                        {{ $assigned?->actual_move_in }}</small>
                                                                @endif
                                                            </div>
                            ...
                            @if($pendingBedLeases->count() > 0)
                                <div class="card border-warning">
                                    ...
                                    <div class="card-body">
                                        <p class="text-muted mb-3">The following leases require bed assignment before the tenant can
                                            move in:</p>
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-2 p-2 bg-light ">
                                                <div>
                                                    ...
                                                    <small class="text-muted">
                                                        {{ $pendingLease->start_date->format('M d, Y') }} -
                                                        {{ $pendingLease->end_date->format('M d, Y') }}
                                                        | ${{ number_format($pendingLease->rent_amount, 2) }}/mo
                                                    </small>
                                                    <br>
                                                    <span
                                                        class="badge bg-{{ $pendingLease->status === 'ACTIVE' ? 'success' : 'info' }}">{{ $pendingLease->status }}</span>
                                                </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title mb-0"><i class="fe fe-user text-primary me-2"></i>Contact Information
                                </h5>
                            </div>
                            <div class="card-body">
                                    ...
                                    <div class="info-item mb-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="ms-3">
                                                ...
                                                <span
                                                    class="fw-semibold">{{ ($profile?->date_of_birth ?? $tenant->date_of_birth)?->format('M d, Y') ?? 'N/A' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        @if($tenant->emergencyContacts && $tenant->emergencyContacts->count() > 0)
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0"><i class="fe fe-phone-call text-danger me-2"></i>Emergency
                                        Contacts</h5>
                                </div>
                                <div class="card-body">
                    ...
                    <div class="col-xl-8 col-lg-7">
                            ...
                            <div class="card current-lease-card">
                                ...
                                <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-md-end">
                                                <div class="text-md-end">
                                                    <h3 class="text-primary mb-0">
                                                        ${{ number_format($activeLease->rent_amount, 2) }}</h3>
                                    ...
                                    <div class="row mt-2">
                                        <div class="col-md-4">
                                            <div class="lease-date-box">
                                                ...
                                                <div class="fw-bold">{{ date('M d, Y', strtotime($activeLease->start_date)) }}
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="lease-date-box">
                                                ...
                                                <div class="fw-bold">{{ date('M d, Y', strtotime($activeLease->end_date)) }}
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="lease-date-box">
                                                ...
                                                <div class="fw-bold text-{{ $daysRemaining < 30 ? 'warning' : 'success' }}">
                                                    {{ round($daysRemaining) }} days</div>
                                            </div>
                                        </div>
                                    </div>
                                    {{-- <div class="mt-3">
                                    ...
                                    <div class="mt-3 d-flex justify-content-start">
                                        ...
                                        <a href="#" class="btn btn-outline-info me-2 d-inline-flex align-items-center"
                                            ...
                                            <tbody>
                                                @foreach($invoices as $invoice)
                                                    ...
                                                    <tr class="invoice-row" data-status="{{ $exactStatus }}">
                                                        ...
                                                        <td>
                                                            <span
                                                                class="text-muted">{{ $invoice->lease?->property?->name ?? 'N/A' }}</span>
                                                        </td>
                                                        <td>
                                                            <span
                                                                class="fw-semibold">${{ number_format($invoice->total_amount, 2) }}</span>
                                                        </td>
                                                        <td>
                                                            ...
                                                            @if($isOverdue)
                                                                <br><small
                                                                    class="text-danger">{{ $invoice->due_date->diffForHumans() }}</small>
                                                            @endif
                                                        </td>
                                                        <td>
                                                            @if($isPaid)
                                                        ...
                                                        <td>
                                                            @if($invoice->balance_due > 0)
                                                                <span
                                                                    class="fw-bold text-danger">${{ number_format($invoice->balance_due, 2) }}</span>
                                                            @else
                        ...
                        @if($tenant->leases->count() > 1 || ($tenant->leases->count() == 1 && !$activeLease))
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0"><i class="fe fe-clock text-secondary me-2"></i>Lease History
                                    </h5>
                                </div>
                        ...
                        <div id="changeBedContent" style="display: none;">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <p class="mb-1"><strong>Property:</strong> <span id="changeBedProperty"></span>
                                            </p>
                                            <p class="mb-0"><strong>Current Bed:</strong> <span id="changeBedCurrent"
                                                    class="badge bg-primary"></span></p>
                                        </div>
                                        <div class="col-md-6">
                                            <p class="mb-1"><strong>Lease Period:</strong> <span
                                                    id="changeBedPeriod"></span></p>
                                            <p class="mb-0"><strong>Monthly Rent:</strong> <span id="changeBedRent"
                            ...
                            <div id="changeBedFormSection">
                                <div class="alert alert-info mb-3">
                                    ...
                                    Select a new bed from the available beds below. The change will be recorded in the lease
                                    history.
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="newBedId" class="form-label"><strong>Select New Bed</strong> <span
                                            ...
                                            <div id="noBedAvailable" class="text-danger mt-2" style="display: none;">
                                                <i class="fe fe-alert-circle me-1"></i> No other beds available in this
                                                property.
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="effectiveDate" class="form-label"><strong>Effective Date</strong>
                                                <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control datepicker2" id="effectiveDate"
                                                name="effective_date" placeholder=" Select a effective date" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="changeBedNotes" name="notes" rows="3"
                                        placeholder="Enter reason or notes for the bed change..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div id="assignBedContent" style="display: none;">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <p class="mb-0"><strong>Property:</strong> <span id="assignBedProperty"></span>
                                            </p>
                                        </div>
                                        <div class="col-md-6">
                                            <p class="mb-1"><strong>Lease Period:</strong> <span
                                                    id="assignBedPeriod"></span></p>
                                            <p class="mb-1"><strong>Monthly Rent:</strong> <span id="assignBedRent"
                                                    class="text-success fw-bold"></span></p>
                                            <p class="mb-0"><strong>Status:</strong> <span id="assignBedStatus"
                                                    class="badge"></span></p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div id="assignBedFormSection">
                                <div class="alert alert-success mb-3">
                                    ...
                                    Select a bed to assign to this lease. Once assigned, the tenant can move in on the
                                    specified date.
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="assignBedSelect" class="form-label"><strong>Select Bed</strong>
                                                <span class="text-danger">*</span></label>
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="moveInDate" class="form-label"><strong>Move-in Date</strong> <span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control datepicker2" id="moveInDate"
                                                name="move_in_date" placeholder="Select move-in date" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="assignBedNotes" name="notes" rows="3"
                                        placeholder="Enter any notes about the bed assignment..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success d-none d-inline-flex align-items-center"
                        id="approveFromModalBtn">
                        <i class="fe fe-check me-1"></i> Approve & Send Form
                    </button>
                    <button type="button" class="btn btn-danger d-none d-inline-flex align-items-center"
