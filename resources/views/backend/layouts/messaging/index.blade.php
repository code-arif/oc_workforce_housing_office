                            ...
                            <ul class="nav1 nav-column flex-column br-7">
                                <li class="nav-item1 mt-0">
                                    <a class="nav-link thumb folder-link {{ $folder == 'inbox' ? 'sidebar-active' : '' }}"
                                       href="{{ route('messaging.index', ['folder' => 'inbox']) }}" data-folder="inbox">
                                ...
                                <li class="nav-item1">
                                    <a class="nav-link thumb folder-link {{ $folder == 'starred' ? 'sidebar-active' : '' }}"
                                       href="{{ route('messaging.index', ['folder' => 'starred']) }}" data-folder="starred">
                                ...
                                <li class="nav-item1">
                                    <a class="nav-link thumb folder-link {{ $folder == 'drafts' ? 'sidebar-active' : '' }}"
                                       href="{{ route('messaging.index', ['folder' => 'drafts']) }}" data-folder="drafts">
                                ...
                                <li class="nav-item1">
                                    <a class="nav-link thumb folder-link {{ $folder == 'trash' ? 'sidebar-active' : '' }}"
                                       href="{{ route('messaging.index', ['folder' => 'trash']) }}" data-folder="trash">
                        ...
                        <div class="card-body p-0">
                            ...
                            <div class="p-3 border-bottom">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ms-auto">
                                        <span class="text-muted">1-{{ $messages->count() }} of {{ $messages->total() }}</span>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="flex-grow-1" style="min-width: 0;">
                                                    ...
                                                    <div class="email-preview text-truncate">
                                                        <span class="fw-semibold">{{ $message->subject ?: '(No Subject)' }}</span>
        ...
        <div class="modal-content">
            ...
            <form id="composeForm">
                <div class="modal-body">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="toEmails"
                               placeholder="To" required>
                        <small class="text-muted">Separate multiple emails with commas</small>
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="ccEmails"
                               placeholder="Cc">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="bccEmails"
                               placeholder="Bcc">
                    </div>
                    <div class="mb-3">
...
$(document).ready(function() {
    ...
    $('#composeForm').submit(function(e) {
        ...
        const toEmails = $('#toEmails').val().split(',').map(e => ({email: e.trim()}));
        const ccEmails = $('#ccEmails').val() ? $('#ccEmails').val().split(',').map(e => ({email: e.trim()})) : [];
        const bccEmails = $('#bccEmails').val() ? $('#bccEmails').val().split(',').map(e => ({email: e.trim()})) : [];
    ...
    $('#saveDraftBtn').click(function() {
        const toEmails = $('#toEmails').val() ? $('#toEmails').val().split(',').map(e => ({email: e.trim()})) : [];
        const ccEmails = $('#ccEmails').val() ? $('#ccEmails').val().split(',').map(e => ({email: e.trim()})) : [];
        const bccEmails = $('#bccEmails').val() ? $('#bccEmails').val().split(',').map(e => ({email: e.trim()})) : [];
