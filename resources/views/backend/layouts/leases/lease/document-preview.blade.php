...
@section('content')
                    ...
                    <div class="ms-auto pageheader-btn">
                        ...
                        @if ($document && !$document->admin_signed_at && $hasAdminSignatureSlot)
                            <button type="button" class="btn btn-success"
                                onclick="openSignatureModal('admin', 'Admin Signature')">
                                <i class="fas fa-pen-fancy"></i> Quick Admin Sign
                            </button>
                        @endif
                        @if ($document)
                            <a href="{{ route('lease-documents.download-pdf', $document->id) }}" class="btn btn-primary"
                                target="_blank">
                                ...
                                <div class="summary-list">
                                    <div class="summary-item mb-3 p-2 rounded-1 hover-light transition-all  bg-light-50">
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <label
                                                class="text-muted small mb-0 text-uppercase fw-bold ls-1">Property</label>
                                        </div>
                                    ...
                                    <div class="summary-item mb-3 p-2 rounded-1 hover-light transition-all  bg-light-50">
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <label class="text-muted small mb-0 text-uppercase fw-bold ls-1">Unit / Room /
                                                Bed</label>
                                        </div>
                                    ...
                                    <div class="summary-item mb-3 p-2 rounded-1 hover-light transition-all  bg-light-50">
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <label class="text-muted small mb-0 text-uppercase fw-bold ls-1">Lease
                                                Term</label>
                                        </div>
                                        <div class="fw-semibold text-dark ps-4">
                                    ...
                                    <div class="summary-item mb-3 p-3 rounded-1 hover-light transition-all  bg-light-50">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <label class="text-primary small mb-0 text-uppercase fw-bold">Seasonal
                                                Rent</label>
                                            <i class="fas fa-money-bill-wave text-primary"></i>
                                        </div>
                                        <div class="fw-bold text-primary fs-3">${{ number_format($lease->rent_amount, 2) }}
                                        </div>
                                    </div>

                                    @if ($lease->deposit_amount > 0)
                                        <div
                                            class="summary-item mb-3 p-2 rounded-1 hover-light transition-all  bg-light-50">
                                            <div class="d-flex align-items-center mb-1">
                                                <i class="fas fa-shield-alt text-muted me-2 small"></i>
                                                <label class="text-muted small mb-0 text-uppercase fw-bold ls-1">Security
                                                    Deposit</label>
                                            </div>
                                            <div class="fw-semibold text-dark ps-4">
                                                ${{ number_format($lease->deposit_amount, 2) }}</div>
                                        </div>
                                    @endif

                                    <div class="summary-item p-2 rounded-1 hover-light transition-all  bg-light-50">
                                        <label class="text-muted small mb-1 d-block text-uppercase fw-bold ls-1">Active
                                            Template</label>
                                        <div class="d-flex align-items-center">
                                            <span
                                                class="badge bg-{{ $template->file_type == 'pdf' ? 'danger' : 'primary' }} me-2">
                                ...
                                <div class="tenant-profile-card p-3 rounded-1 hover-light transition-all  bg-light-50">
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="avatar-wrapper position-relative me-3">
                                            <img src="{{ $avatar }}" alt="Tenant" class="rounded-circle shadow-sm"
                                                width="60" height="60"
                                                style="object-fit: cover; border: 2px solid #fff;">
                                            <span
                                                class="status-indicator bg-success position-absolute bottom-0 end-0 rounded-circle"
                                                style="width: 12px; height: 12px; border: 2px solid #fff;"></span>
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden">
                                            <h6 class="mb-0 fw-bold text-truncate">{{ $tenantName }}</h6>
                                            <small
                                                class="text-muted text-truncate d-block">{{ $lease->tenant?->email ?? 'N/A' }}</small>
                                        </div>
                                    </div>
                                    <div class="tenant-contact-info border-top pt-3 mt-1">
                                        @if ($profile?->phone)
                                            <div class="d-flex align-items-center mb-2 small text-dark">
                                                <div class="bg-white rounded-circle shadow-xs p-1 me-2 text-center"
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    @if ($document)
                                        @if ($document->tenant_signed_at && $document->admin_signed_at)
                                            <span class="badge bg-success"><i class="fas fa-check-circle me-1"></i> Fully
                                                Signed</span>
                                        @elseif($document->tenant_signed_at)
                                            <span class="badge bg-info"><i class="fas fa-clock me-1"></i> Pending Admin
                                                Signature</span>
                                        @elseif($document->admin_signed_at)
                                            <span class="badge bg-warning"><i class="fas fa-clock me-1"></i> Pending Tenant
                                                Signature</span>
                                        @else
                                            <span class="badge bg-secondary"><i class="fas fa-edit me-1"></i> Pending
                                                Signatures</span>
                                        @endif
                                    @endif
    ...
    <div id="signatureModal" class="signature-modal">
        <div class="signature-modal-content">
            ...
            <div class="signature-modal-actions d-flex gap-2 justify-content-end">
                <button type="button" class="btn btn-light btn-sm px-3 py-1 border" onclick="clearSignature()">
                    <i class="fas fa-eraser me-1 small"></i>
                    Clear
                </button>

                <button type="button" class="btn btn-light btn-sm px-3 py-1 border text-danger"
                    onclick="closeSignatureModal()">
                    <i class="fas fa-times me-1 small"></i>
                    Cancel
                </button>

                <button type="button" class="btn btn-dark btn-sm px-3 py-1" onclick="saveSignature()"
                    id="saveSignatureBtn">
    ...
    <div id="textInputModal" class="signature-modal">
        <div class="signature-modal-content" style="max-width: 600px;">
            ...
            <div class="mb-3">
                ...
                <textarea id="customTextArea" class="form-control" rows="6" placeholder="Enter your text here..."
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderPdf() {
                container.innerHTML =
                    '<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-2x"></i><p class="mt-2">Loading PDF...</p></div>';
                ...
                loadingTask.promise.then(function(pdf) {
                    ...
                    container.innerHTML =
                        '<div class="alert alert-danger"><i class="fas fa-exclamation-circle me-2"></i>Failed to load PDF: ' +
                        error.message + '</div>';
            ...
            function createSignatureOverlay(s, scale) {
                ...
                if (isSigned && signatureData) {
                    ...
                    clickIcon.innerHTML = '<i class="fas fa-pen-fancy" style="font-size: ' + (16 * scale) +
                        'px; color: #f59e0b;"></i>';
                ...
                fetch(endpoint, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                title: 'Error!',
                                text: data.message || 'Failed to save signature.',
                                icon: 'error',
                                confirmButtonColor: '#ba9779'
                            });
                        }
            ...
            window.saveTextInput = function() {
                ...
                fetch('{{ route('lease-documents.update-custom-fields', ':id') }}'.replace(':id',
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: 'Failed to save text. Please try again.',
                            icon: 'error',
                            confirmButtonColor: '#ba9779'
                        });
                        customTextValues[currentTextFieldId] = ''; // Revert
...
@push('styles')
    <style>
        ...
        /* Ensure SweetAlert2 is always on top of signature pad modal */
        .swal2-container {
